...
public class Emotion {
    ...
    static {
        try {
            Haos = new HashSet<>(JSON.parseObject(IOUtils.toString(Emotion.class.getResourceAsStream("/senti/好.json"), StandardCharsets.UTF_8), new TypeReference<List<String>>() {}));
            Les = new HashSet<>(JSON.parseObject(IOUtils.toString(Emotion.class.getResourceAsStream("/senti/乐.json"), StandardCharsets.UTF_8), new TypeReference<List<String>>() {}));
            Ais = new HashSet<>(JSON.parseObject(IOUtils.toString(Emotion.class.getResourceAsStream("/senti/哀.json"), StandardCharsets.UTF_8), new TypeReference<List<String>>() {}));
            Nus = new HashSet<>(JSON.parseObject(IOUtils.toString(Emotion.class.getResourceAsStream("/senti/怒.json"), StandardCharsets.UTF_8), new TypeReference<List<String>>() {}));
            Jus = new HashSet<>(JSON.parseObject(IOUtils.toString(Emotion.class.getResourceAsStream("/senti/惧.json"), StandardCharsets.UTF_8), new TypeReference<List<String>>() {}));
            Wus = new HashSet<>(JSON.parseObject(IOUtils.toString(Emotion.class.getResourceAsStream("/senti/恶.json"), StandardCharsets.UTF_8), new TypeReference<List<String>>() {}));
            Jings = new HashSet<>(JSON.parseObject(IOUtils.toString(Emotion.class.getResourceAsStream("/senti/惊.json"), StandardCharsets.UTF_8), new TypeReference<List<String>>() {}));

            // 读取停用词表文件
            stopWords = IOUtils.readLines(Emotion.class.getResourceAsStream("/senti/StopWords.txt"), StandardCharsets.UTF_8).stream().collect(Collectors.toSet());
        } catch (IOException e) {
            throw new RuntimeException("Error initializing Emotion class", e);
        }
